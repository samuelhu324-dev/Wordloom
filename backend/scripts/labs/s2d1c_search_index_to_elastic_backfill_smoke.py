"""S2D-1C/P1-C1-S1: search_index_to_elastic backfill smoke (skeleton).

This lab is a skeleton placeholder for the legacy projection
`search_index_to_elastic` backfill smoke drill. It is intentionally
"known red" in v1:
...
- It writes `<outdir>/_result.json` with a minimal evidence payload
  compatible with the S2D-1A/1B style.
- It always reports `ok=false` with a low-cardinality reason
  `"skeleton_not_implemented_yet"` and exits with status code 2.

Later cycles (P2/C2+) can replace this implementation with a real
backfill smoke that exercises the outbox → Elastic path.
"""
...
SCENARIO_ID = "verify/search_index_to_elastic/backfill_smoke_skeleton"
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="S2D-1C search_index_to_elastic backfill smoke (skeleton)",
    )
    p.add_argument(
        "--database-url",
        required=True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = EvidenceResult(
        ...
        notes={
            "message": "S2D-1C skeleton only; backfill smoke not implemented yet",
        },
    )
    ...
    print(
        "[S2D-1C] backfill smoke skeleton wrote _result.json; "
