"""S2D-1A/P2-C1-S2: chronicle_daily_stats harness correctness drill (local, DB-only).

This lab exercises the projection framework harness for the
`chronicle_daily_stats` projection by:

- Creating (or reusing) a minimal Library/Bookshelf/Book FK chain
- Writing one Chronicle event via SQLAlchemyChronicleRepository
- Emitting one unified outbox row for `projection="chronicle_daily_stats"`
  via the generic backfill template
- Running the projection harness until idle for this projection
- Verifying the outbox row was processed successfully (status='done')

Outputs:
- Writes `<outdir>/_result.json` as the evidence SoT for the round.

Notes:
- DB-only by design; the projection currently has a v0 adapter which
  validates payload contract and derives an in-memory aggregation key.
"""
...
async def _create_chronicle_event(*, database_url: str) -> dict[str, str]:
    ...
    async with session_factory() as session:
        ...
        ev = ChronicleEvent.create(
            ...
            book_id=book_id,
            payload={"schema_version": 1, "provenance": "s2d1a_daily_stats_harness_drill"},
        )
...
async def _backfill_once(*, session_factory: Any, entity_id: UUID, event_version: int, run_id: str) -> BackfillStats:
    async def _work(session: Any, emit: BackfillEmitter) -> None:
    ...
    _, stats = await run_backfill(
        projection_name=PROJECTION_NAME,
        session_factory=session_factory,
        work=_work,
        run_id=run_id,
        worker_id=f"s2d1a_daily_stats_harness_drill:{run_id}",
        dry_run=False,
        batch_size=100,
    )
    return stats
...
def _count_outbox_by_status(*, database_url: str, entity_id: str) -> dict[str, int]:
    cs = _database_url_psycopg(database_url)
    with psycopg.connect(cs) as conn:
        conn.execute("SET TIME ZONE 'UTC'")
        with conn.cursor() as cur:
            cur.execute(
                ...
                GROUP BY status
                """,
                (PROJECTION_NAME, LAB_ENTITY_TYPE, entity_id),
            )
...
class EvidenceResult:
    ...
    outbox_before: dict[str, int]
    outbox_after: dict[str, int]
...
async def _run(*, database_url: str, run_id: str, outdir: Path) -> EvidenceResult:
    ...
    # Emit one outbox row for this projection.
    stats = await _backfill_once(
        session_factory=session_factory,
        entity_id=entity_id,
    ...
    before = _count_outbox_by_status(database_url=database_url, entity_id=str(entity_id))

    # Run harness until idle for this projection.
    cfg = HarnessConfig(
        batch_size=50,
        lease_seconds=15.0,
    ...
    after = _count_outbox_by_status(database_url=database_url, entity_id=str(entity_id))

    done_after = int(after.get("done") or 0)
    failed_after = int(after.get("failed") or 0)
    pending_after = int(after.get("pending") or 0)

    ok = harness_exit_code == 0 and done_after >= 1 and failed_after == 0 and pending_after == 0

    result = EvidenceResult(
        lab_id="s2d1a_chronicle_daily_stats_harness_drill",
        scenario="verify/chronicle_daily_stats/harness_drill",
        run_id=run_id,
        created_at=_utc_now_str(),
        ok=bool(ok),
        database_url=database_url,
        chronicle=chronicle,
        backfill_stats=_stats_jsonable(stats),
