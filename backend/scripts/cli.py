...
def _cmd_labs_shadow_verify_dual_run_stage1(args: argparse.Namespace) -> int:
    ...
    def _sanitize_index_name(name: str) -> str:
        # ES index must be lowercase and avoid special chars.
    ...
    seed_entity_type = "block"  # Postgres FTS provider is block-only.
    ...
    engine = create_engine(database_url)
    with engine.connect() as conn:
        # Seed specifically for this drill's query (block-only + token match).
    ...
    child_env["ELASTIC_URL"] = es_url
    child_env["ELASTIC_INDEX"] = es_index
    ...
    try:
        ...
        with tracer.start_as_current_span(span_name) as span:
            ...
            span.set_attribute("lab_id", LAB_ID_S2B_2A_2A)
            span.set_attribute("scenario", SCENARIO_SHADOW_VERIFY_DUAL_RUN_STAGE1)
            span.set_attribute("run_id", run_id)
            span.set_attribute("scope", scope)
            span.set_attribute("library_id", library_id or "")
    ...
    result: dict[str, object] = {
        ...
        "elasticsearch": {
            ...
            "backfill": {
                ...
                "ok": bool(backfill_ok),
                "log_path": _rel_repo(backfill_log_path),
