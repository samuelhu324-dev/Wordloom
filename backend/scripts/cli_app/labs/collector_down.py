...
def _invoke_scenario(
    *,
    ...
    fallback_exit_code: Callable[[bool], int] | int,
) -> int:
...
def cmd_labs_run_collector_down(
    ...
    *,
    ...
    return _invoke_scenario(
        handler_name=f"{handler_base}.run",
        payload=payload,
        fallback_exit_code=lambda ok: 0 if ok else 2,
    )


def cmd_labs_verify_collector_down(
    args: argparse.Namespace,
    *,
    scope_id: str,
    scenario: str,
    ...
    return _invoke_scenario(
        handler_name=f"{handler_base}.verify",
        payload=payload,
        fallback_exit_code=lambda ok: 0 if ok else 10,
    )


def cmd_labs_export_collector_down(
    args: argparse.Namespace,
    *,
    scope_id: str,
    scenario: str,
    ...
    return _invoke_scenario(
        handler_name=f"{handler_base}.export",
        payload=payload,
        fallback_exit_code=0,
    )


def cmd_labs_clean_collector_down(
    args: argparse.Namespace,
    *,
    scope_id: str,
    handler_base: str,
    ...
    return _invoke_scenario(
        handler_name=f"{handler_base}.clean",
        payload=payload,
        fallback_exit_code=0,
    )
