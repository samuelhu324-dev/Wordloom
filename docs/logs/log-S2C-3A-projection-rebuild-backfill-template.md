...
- `P2-C1-S2`（rebuild smoke）在 commit `f3fdfedc` 完成：新增 Chronicle rebuild smoke 场景 `verify/chronicle/rebuild_entries_smoke`，runner 为 `backend/scripts/labs/s2c3a_rebuild_chronicle_entries_smoke.py`（DB-only；产物 `_result.json` 为 SoT）
