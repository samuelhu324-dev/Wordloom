...
| `DOC-WORKFLOW-LEGACY-LOGS-0001` | `docs-governance` | `delegated:workflow-legacy-logs-contract-maintainer` | `superseded-historical-release` | `role:workflow-reviewer` | `role:docs-governance-approver` | The logs layer now also has one governed historical-release surface for the earliest structured-log shape, while the later current logs-family reader remains separate at `DOC-WORKFLOW-LOGS-0001`. |
...
- This block records current effective governance state for the parent ledger, the governed historical logs release now attached to `R02`, and the narrow labs, runbook, and ADR children.
- Detailed evidence verification, direct-markdown review, and packet write-back history remain in `ledger-SUP-S0A-2A-001`, `ledger-SUP-S0A-2A-002`, `ledger-SUP-S0A-2A-003`, and `ledger-SUP-S0A-2A-004` rather than being flattened into current-state ownership metadata.
...
| `S0A-2A-R02` | `Logs layer` in issue `S0A-2A (#24)` | logs convert raw material into structured plans with status, what, how, and links | `DOC-WORKFLOW-LEGACY-LOGS` | `historical-backfill` | `none-source-only` | `keep-in-issue` | `applied` | `DOC-WORKFLOW-LEGACY-LOGS-0001` | `historical-review` | accepted `004` logs SUP evidence now fixes one governed historical-only logs release for this slice, while the later `DOC-WORKFLOW-LOGS-0001` family remains a separate current reader for a different logs-facing rule body and now explicitly points back to that earlier reader as predecessor context only | This slice no longer stays at bounded background only; it now records one explicit earlier structured-log shape through the accepted packet-level accountability chain rather than by direct source-path write-back alone. |
...
- later reciprocal bridge write-back on `DOC-WORKFLOW-LOGS-0001` after the historical-only legacy logs release is reviewed
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `S0A-2A-R01` | `unknown` | `2026-02-06T06:03:03Z` | `2026-02-06T06:03:03Z` | `ongoing` | `second` | `source issue preserves UTC-second creation time` | The broad workflow parent row is anchored directly to the source issue creation time because the issue body itself already defends the one-way refinement pipeline and source-linking rule. |
| `S0A-2A-R02` | `unknown` | `unknown` | `unknown` | `unknown` | `unknown` | `the accepted logs SUP packet preserves structure and provenance but not a defended unified execution timestamp across both earliest retained logs` | The parent row now records one defended earlier logs review state, but the row-level chronology still depends on the more specific `SUP-004` packet for any narrower time audit. |
| `S0A-2A-R03` | `2026-02-03` | `unknown` | `2026-02-03` | `ongoing` | `day` | `accepted labs SUP evidence currently proves one defended day-level execution date for labs-004 only` | The parent row now records one defended earlier-labs review state and one narrow day-level source anchor through `S0A-2A-R03-SUP-01`, but the broader row still remains only partially reconstructed because other labs evidence in the same slice is still chronology-incomplete. |
| `S0A-2A-R04` | `unknown` | `2026-02-06T06:03:03Z` | `2026-02-06T06:03:03Z` | `ongoing` | `second` | `source issue preserves UTC-second creation time; accepted runbook SUP later sharpens the child-opening read` | The runbook row now distinguishes the issue-created source anchor from the later direct-evidence sharpening packet: the issue already defends the runbook layer, while `SUP-001` sharpens the narrower child-opening justification. |
| `S0A-2A-R05` | `unknown` | `2026-02-06T06:03:03Z` | `2026-02-06T06:03:03Z` | `ongoing` | `second` | `source issue preserves UTC-second creation time; accepted ADR SUP later sharpens the child-opening read` | The ADR row now distinguishes the issue-created source anchor from the later direct-evidence sharpening packet: the issue already defends the ADR layer, while `SUP-003` sharpens the narrower child-opening justification. |
...
| `S0A-2A-GOV-11` | `evidence-sharpening-event` | `S0A-2A-R02 logs layer` | `role:packet-reviewer` | `logs-legacy-historical-review-fixed` | `2026-04-22` | `ledger-SUP-S0A-2A-004-legacy-logs-earliest-structured-shape.md` | The accepted logs SUP round now fixes `R02` as one explicit historical-review slice under `DOC-WORKFLOW-LEGACY-LOGS-0001` instead of leaving the logs layer as bounded background only or relying on a direct source-path write-back alone. |
...
- This ledger now confirms that `S0A-2A` remains primarily parent-owned, while the logs layer now sits in explicit historical review under `DOC-WORKFLOW-LEGACY-LOGS-0001`, the labs layer now sits in explicit historical review under `DOC-WORKFLOW-LABS-0002`, the runbook layer is consumed by one dedicated child contract, and the ADR layer is now consumed by one dedicated child contract.
...
- The row chronology audit now distinguishes three cases: source issue time can directly anchor the broad workflow, runbook, and ADR rows; the labs row has one narrower defended `2026-02-03` SUP-backed anchor but is still only partially reconstructed; and the logs row remains chronology-deferred because accepted `SUP-004` still proves reader shape more strongly than a stable historical execution timestamp.
- The logs slice remains special: accepted `SUP-004` now feeds `S0A-2A-R02`, `DOC-WORKFLOW-LEGACY-LOGS-0001` records the earlier historical reader for the earliest structured-log shape, and the later current logs-family reader remains separate at `DOC-WORKFLOW-LOGS-0001` while pointing back to that historical reader as predecessor context only.