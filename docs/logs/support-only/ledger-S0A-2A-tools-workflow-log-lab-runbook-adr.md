...
| `S0A-2A-R02` | `Logs layer` in issue `S0A-2A (#24)` | logs convert raw material into structured plans with status, what, how, and links | `DOC-WORKFLOW-LEGACY-LOGS` | `historical-backfill` | `none-source-only` | `keep-in-issue` | `applied` | `DOC-WORKFLOW-LEGACY-LOGS-0001` | `historical-review` | the earliest direct legacy-log evidence now fixes one governed historical-only logs release for this slice, while the later `DOC-WORKFLOW-LOGS-0001` family remains a separate current reader for a different logs-facing rule body | This slice no longer stays at bounded background only; it now records one explicit earlier structured-log shape without claiming that the later logs family was sourced directly from this broad issue. |
...
| `S0A-2A-GOV-11` | `evidence-sharpening-event` | `S0A-2A-R02 logs layer` | `role:packet-reviewer` | `logs-legacy-historical-review-fixed` | `2026-04-22` | `legacy/from_structured_docs/from-logs/v2-logs/log-S0A-dlq-replay-platform.md; legacy/from_structured_docs/from-logs/v2-logs/log-S0B-graceful-termination+heathz-readyz+alert-threshold.md` | The earliest direct legacy structured-log evidence now fixes `R02` as one explicit historical-review slice under `DOC-WORKFLOW-LEGACY-LOGS-0001` instead of leaving the logs layer as bounded background only. |
...
- The logs slice remains special: `DOC-WORKFLOW-LEGACY-LOGS-0001` records the earlier historical reader for the earliest structured-log shape, while the later current logs-family reader still remains separate at `DOC-WORKFLOW-LOGS-0001`.