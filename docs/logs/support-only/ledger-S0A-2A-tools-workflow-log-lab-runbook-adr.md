...
| `S0A-2A-R04` | `Runbook layer` in issue `S0A-2A (#24)` | runbooks distill invariants from labs into operator-facing troubleshooting and recovery guidance | `DOC-WORKFLOW-RUNBOOK` candidate | `new-family` | `none-source-only` | `defer` | `deferred` | `none` | `none` | direct runbook evidence from `ledger-SUP-S0A-2A-tools-workflow-log-lab-runbook-adr.md` now justifies explicit child-candidate review, so this slice should no longer remain bounded background only; actual release opening stays deferred until a defended runbook packet is promoted | The earliest projection SOPs now prove durable operator-facing runbook extraction beyond the broad issue summary alone, but no runbook contract has been opened yet. |
