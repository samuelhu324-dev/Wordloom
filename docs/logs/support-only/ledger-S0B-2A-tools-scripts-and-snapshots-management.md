...
| `3) Snapshots policy` labs sub-slice in [log-S0B-2A-scripts-snapshots-management.md](d:/Project/wordloom-v3/docs/logs/log-S0B-2A-scripts-snapshots-management.md) | labs evidence-root semantics, run folder shape, retention, and auditability for lab snapshot outputs | `DOC-WORKFLOW-LABS` | `new-release` | `none-source-only` | `keep-in-log` | `draft` | `none` | `none` | this is the strongest current candidate for later consumption by `DOC-WORKFLOW-LABS-0002`, but no release has been written yet | This is the cleanest first release-sample path because it materially extends the existing labs family without forcing unrelated scripts-governance slices into the same release. |
...
## First-Sample Recommendation

- The first release sample from this ledger should still be one later release in the `DOC-WORKFLOW-LABS` family.
- The current likely sample shape is:
  - existing release: `DOC-WORKFLOW-LABS-0001`
  - later release candidate: `DOC-WORKFLOW-LABS-0002`
- `DOC-WORKFLOW-LABS-0002` should absorb only the `S0B-2A` slice that materially strengthens labs snapshot governance:
  - unified labs evidence root
  - run-folder shape
  - retention or archival guidance for labs outputs
  - clearer evidence-package semantics
- The first sample should not, by default, absorb the scripts taxonomy, stable CLI entrypoint, or cutover/stub slices into `LABS-0002`.
