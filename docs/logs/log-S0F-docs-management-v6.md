...
- `S0F-7A/P3-C4-S1S2` is now complete in workspace: the repo now also has one broader `DOC-WORKFLOW-0001` workflow-layer draft from issue `S0A/2A`, so the broader workflow pipeline contract can be judged separately from the narrower GitHub-issues packet.
