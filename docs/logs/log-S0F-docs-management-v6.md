...
- `S0F-7D/P1-C3-S1` is now complete in workspace: on-disk supplement-ledger artifact naming now uses the shorter `SUP` prefix, and both the reusable template and first pilot file now follow that same naming rule.
- `S0F-7D/P2-C1-S1` is now complete in workspace: the first screenshot-backed Projects SUP pilot now exists at stable repo-local paths under `docs/logs/support-only/`, and the current reading keeps the Projects routing intact while preparing write-back that sharpens the parent row and the current Projects draft.
