...
- `S0F-1D/P1` is now complete: create-time issue completeness is fixed for body plus sidebar coverage, blank optional `Links` rows are explicitly non-canonical, and the next follow-up is `P2` PR development-linkage completeness.
