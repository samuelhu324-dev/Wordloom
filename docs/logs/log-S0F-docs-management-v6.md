...
- `S0F-7E/P3-C1-S1S2` is now complete in workspace: the repo now has one explicit `historical-backfill` release action plus one minimum lineage-update rule for later-recorded earlier states, so earlier history can be admitted without family renumbering or mandatory whole-family rewrites.
