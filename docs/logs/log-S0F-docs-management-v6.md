...
- `S0F-7E/P2-C1-S1S2` is now complete in workspace: the contract template now separates registry order from recorded and historical-effective time, and statement tables now expose chronology range fields rather than forcing release ids to act as time proxies.
