...
- `S0F-7A/P3-C5-S1S2` is now complete in workspace: the repo now also has one narrower `DOC-WORKFLOW-LABS-001` labs-layer draft from issue `S0B/1A`, so labs/snapshots governance can be judged separately from both the broader workflow contract and the GitHub-issues packet.
