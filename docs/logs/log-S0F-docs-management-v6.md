...
- `S0F-7D/P2-C8-S1S2S3` is now complete: the `S0A-2A` selective-backfill ledger now exposes exact row ids for later SUP anchoring, the first runbook-direct-evidence SUP round now attaches the earliest search and chronicle projection SOPs to the runbook layer, and the accepted parent-row write-back now records explicit runbook child-candidate review without pretending a `DOC-WORKFLOW-RUNBOOK` contract already exists.
