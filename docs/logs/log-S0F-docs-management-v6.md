...
- `S0F-7B/P2-C1-S1S2S3` is now complete in workspace: the repo now reads supersede, split, absorb, and merge as release-level lineage semantics, while logs/issues/support-only routing stays owned by the ledger rather than by contract lineage fields.
