...
- `S0F-7A/P3-C2-S1S2` is now complete in workspace: the rejected broad four-contract draft has been withdrawn, and the repo now keeps one replacement `S0A-1A` issue-first preview contract that states issue breakdown, title encoding, and tag naming governance directly while keeping the issue-only source boundary explicit.
- `S0F-7A/P3-C3-S1S2S3` is now complete in workspace: the temporary mixed `S0A-1A` preview has been replaced by one parent GitHub-issues mechanism contract plus one title child contract and one tag child contract under the new long-path naming grammar.
