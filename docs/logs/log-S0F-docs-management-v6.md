...
- `S0F-1D/P3` is now complete: conclusion-time completeness is fixed for final issue body plus sidebar re-check, child versus top-level parent close-out rules are explicit, and the next follow-up is `P4` read-only audit packaging.
