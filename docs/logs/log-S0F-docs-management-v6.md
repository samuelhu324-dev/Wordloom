...
- `S0F-7D/P2-C3-S1S2` is now complete in workspace: the repo now has one optional contract statement-table model for clause-level identity, and `LABS-0001` plus the `S0B-1A` parent ledger now serve as the first concrete sample of contract-clause linkage back to ledger row ids.
- `S0F-7D/P2-C4-S1S2` is now complete in workspace: contract statement tables now distinguish short `statement label` values from ledger-owned `source slice` values, and `source basis` is now fixed as one-or-more stable anchors under a primary-first rule.
- `S0F-7D/P2-C5-S1S2S3` is now complete in workspace: `source basis` is now narrowed back to evidence-only anchors, the repo now has one explicit `Statement Evolution Table` model for clause lineage, and `LABS-0001` now demonstrates the paired-table split sample.
