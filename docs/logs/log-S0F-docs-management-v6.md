...
- `S0F-7D/P2-C6-S1S2S3` is now complete in workspace: later-release statement ids are now fixed as release-local ids, the `S0B-2A` ledger now exposes row ids for `LABS-0002`, and `LABS-0002` now follows the same paired-table clause model used by `LABS-0001`.
- `S0F-7D/P2-C7-S1S2` is now complete in workspace: current statement tables now sort by older semantic origin first, and `LABS-0002` now preserves `0001` as the first-effective origin for carried-forward and amended clauses.
