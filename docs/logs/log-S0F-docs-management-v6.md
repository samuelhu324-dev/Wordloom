...
- `S0F-7E/P4-C1-S1` is now complete in workspace: the original `S0A-2A` runbook SUP round now reads as sequence `001`, and one new `002` labs SUP round now exists under the same supplement series without any parent-row write-back yet.
