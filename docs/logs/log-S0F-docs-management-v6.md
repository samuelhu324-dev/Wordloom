...
- `S0F-7D/P1-C2-S1S2S3` is now complete in workspace: stable parent-ledger row ids, supplement item ids, and screenshot or attachment ids are now fixed, the `S0A-1A` parent ledger now exposes explicit row anchors, and those ids are now kept below the contract layer rather than promoted into release identity.
