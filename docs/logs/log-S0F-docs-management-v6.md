...
- `S0F-7B/P2-C2-S1S2` is now complete in workspace: the repo now explicitly separates family hierarchy from release split and no longer uses split solely to express the broader `WORKFLOW` versus narrower `WORKFLOW-LABS` family relationship.
...
- `S0F-7B/P5-C2-S1S2` is now complete in workspace: the repo now has one drafted `DOC-WORKFLOW-LABS-0002` release from `LABS-0001` plus the consumed labs-only `S0B-2A` ledger slice, while non-labs slices remain deferred or support-only.
