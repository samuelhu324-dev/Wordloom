...
- `S0F-6A` is now complete as the bounded follow-up for reader-summary enhancement on the migration view: `view-old-s0-migration-ledger-v1.md` now groups the already-admitted `DOC` absorption state by `contract`, `history view`, and `promotion-map view`, each group carries a fixed `open first -> then open` handoff, and one compact question-first decision block now routes readers by intent before they even enter the grouped classes.
