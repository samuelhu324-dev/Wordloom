...
- `S0F-7C/P2-C1-S1S2S3` is now complete in workspace: the repo now fixes the first `S0B-3A` child targets as `DOC-WORKFLOW-LOGS-0001` and `DOC-WORKFLOW-LIFECYCLE-0001`, keeps front matter narrow on first extraction with later widening reserved, and splits cutover into separate logs-intake and lifecycle-boundary slices.
