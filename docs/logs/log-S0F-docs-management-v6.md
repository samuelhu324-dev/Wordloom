...
- `S0F-7A/P1-C2-S1S2S3` is now complete in workspace: chronology-first rebuild now also fixes one long-path canonical contract naming grammar, one parent-versus-child contract split model, and one explicit later boundary for generating parent/title/tag template-backed contract packets.
