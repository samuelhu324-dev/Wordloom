...
- `S0F-7E/P5-C1-S1S2` is now complete in workspace: the contract template now gives `history-backfilled` clause rows first ordering priority inside one later current release, and the local `DOC-WORKFLOW-LABS-0002` sample now demonstrates that merged earlier-history reading together with the minimum contract chronology fields.
