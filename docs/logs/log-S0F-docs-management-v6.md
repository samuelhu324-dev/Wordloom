...
- `S0F-7D/P0-C1-S1S2` is now complete in workspace: the repo now has one bounded continuation lane after `7C` for supplement-ledger admission, and later evidence is now explicitly required to attach to an existing parent source-owned ledger before it can influence contract reading.
- `S0F-7D/P1-C1-S1S2S3` is now complete in workspace: supplement-ledger naming, minimum header, evidence-row contract, and allowed verdict effects are now fixed, and the first reusable supplement-ledger template is now published for pilot use.
