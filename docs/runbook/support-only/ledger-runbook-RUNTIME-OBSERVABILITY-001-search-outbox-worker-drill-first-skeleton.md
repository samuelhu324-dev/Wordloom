...
- Scenario-level routing remains explicit under `RBL-02`, so reviewers can now distinguish which scenario landed in the runbook, which stayed support-only, and which still awaits a later sibling lane.