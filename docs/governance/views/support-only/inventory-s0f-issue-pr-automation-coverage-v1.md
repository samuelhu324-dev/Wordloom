...
- Coverage counts currently stand at:
  - `13` rows with `issue+pr-linked`
  - `1` row with `issue-only`
  - `0` rows with `pr-only`
  - `44` rows with `missing-both`
- Coverage now remains concentrated in `S0F-1*` plus the admitted `S0F-2*` and `S0F-6*` packets; current historical series from `S0F-3*`, `S0F-4*`, `S0F-5*`, and `S0F-7*` remain uncovered in frontmatter.
