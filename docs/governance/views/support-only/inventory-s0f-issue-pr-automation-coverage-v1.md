...
- Coverage counts currently stand at:
  - `14` rows with `issue+pr-linked`
  - `1` row with `issue-only`
  - `0` rows with `pr-only`
  - `43` rows with `missing-both`
- Coverage now remains concentrated in `S0F-1*` plus the admitted `S0F-2*`, `S0F-6*`, and first landed `S0F-4B` subpacket; current historical series from `S0F-3*`, `S0F-5*`, and `S0F-7*` remain uncovered, while `S0F-4*` is now partially covered.
...
| `4` | `9` | `1` | `0` | `0` | `8` | `historical-reviewable` | first admitted subpacket `S0F-4B` is now fully concluded: `#394 / PR #395`; continue bounded subpacket review across `4A` and `4C-4I` |
...
| `4` | `admitted-automated` | `S0F-4B` | `1` | `present` | `present` | `historical-reviewable` | first admitted `S0F-4*` subpacket now has live issue/PR coverage, merged PR evidence, and concluded issue body |
...
| `4` | `missing-series` | `S0F-4A, S0F-4C, S0F-4D, S0F-4E, S0F-4F, S0F-4G, S0F-4H, S0F-4I` | `8` | `missing` | `missing` | `historical-reviewable` | coherent docs-governance packet is now opened through `S0F-4B`; continue commit-readiness review with the remaining bounded subpackets |
