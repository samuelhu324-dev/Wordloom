...
| `S0E-5B` | `S0E` | `no` | `retained-evidence` | `repo guarded lifecycle and apply planning surfaces` | `scripts/issues/plan_lifecycle_remediation.py` plus `docs/runbook/run-S0E-log-to-issue-creation.md` | `retained guarded-lifecycle expansion evidence` | `the row records guarded lifecycle apply expansion, while current operator reading now starts from the live lifecycle planners and runbook procedure rather than from this source log as one separate DOC current home` |
| `S0E-5C` | `S0E` | `no` | `retained-evidence` | `repo PR create gate and preflight planning surfaces` | `scripts/issues/plan_pr_create_preflight_with_gate.py` plus `docs/runbook/run-S0E-log-to-issue-creation.md` | `retained guarded PR-create decomposition evidence` | `the row remains bounded PR-create decomposition evidence, while current guarded PR-create behavior now reads through the live gate/planning surfaces rather than through the source log as one DOC first-open target` |
| `S0E-5D` | `S0E` | `no` | `retained-evidence` | `repo body-contract and completeness-gate surfaces` | `scripts/issues/plan_body_completeness_check_wrapper.py`, `scripts/issues/verify_live_pr_body_contract.py`, and `docs/runbook/run-S0E-log-to-issue-creation.md` | `retained body-contract normalization evidence` | `the row records body-contract and gate-shape normalization, while current body-check and verification behavior now reads through the live check surfaces rather than through the source log as one separate DOC current home` |
| `S0E-5E` | `S0E` | `no` | `history-lineage` | `repo parent-issue DoD ordering and body-boundary surfaces` | `docs/logs/log-S0E-6F-issue-body-metadata-links-boundary-follow-up.md` plus current issue-body procedures | `lineage for parent-issue ordering boundary` | `the row remains historically relevant because its parent-issue DoD ordering rule survives mainly as lineage into later issue-body and ordering surfaces rather than as one first-open current reading surface` |
...
| `S0E-6B` | `S0E` | `no` | `history-lineage` | `repo log-stability and body-gate strategy surfaces` | `docs/logs/log-S0E-6A-log-structure-normalization-and-dual-track-evidence-contract.md` plus later issue/lifecycle gate procedures | `lineage for log-stability gate strategy` | `the row remains historically relevant because it records a broader log-stability and gate strategy that later reads through more concrete body/lifecycle gate surfaces rather than through this source log as one current first-open home` |
