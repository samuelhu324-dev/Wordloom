...
- Scenario-level routing remains explicit under `CRL-02`, so reviewers can now distinguish which scenario already lands in the contract, which current-family scenarios remain release-ledger-only by deliberate no-op decision, and which still await a later sibling lane.