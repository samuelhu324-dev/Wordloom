...
contract_record:
  ...
  notes:
    ...
    - `effective_from` is anchored to the source issue creation time `2026-02-06T06:03:03Z` because the issue body itself already defends the one-way refinement pipeline and source-linking boundary.
```
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `DOC-WORKFLOW-0001-GOV-01` | `contribution-event` | `DOC-WORKFLOW-0001` | `unknown` | `family-introduced` | `2026-02-06T06:03:03Z` | `GitHub issue S0A-2A (#24)` | The issue-only source introduced the broad workflow boundary on the issue creation date, but it does not by itself prove the current steward or approval chain for the current contract state. |
...
| `DOC-WORKFLOW-0001-GOV-06` | `current-boundary-sharpened` | `DOC-WORKFLOW-0001` | `role:packet-reviewer` | `logs-child-boundary-fixed` | `2026-04-22` | `ledger-SUP-S0A-2A-004-legacy-logs-earliest-structured-shape.md; ledger-S0A-2A-tools-workflow-log-lab-runbook-adr.md; DOC-WORKFLOW-LEGACY-LOGS-0001; DOC-WORKFLOW-LOGS-0001` | The current parent boundary now reads the logs clause as one delegated summary: the parent keeps the broad workflow-layer logs role, the earlier historical logs reader sits at `DOC-WORKFLOW-LEGACY-LOGS-0001`, and the later current logs-family reader sits at `DOC-WORKFLOW-LOGS-0001`. |
...
| `DOC-WORKFLOW-0001-BND-03` | `DOC-WORKFLOW-0001-ST-03` | `delegated-summary` | `DOC-WORKFLOW-LOGS-0001` | one broad workflow reading of what the logs layer is for | The parent now keeps only the broad workflow-layer logs boundary; read `ledger-S0A-2A` plus `DOC-WORKFLOW-LEGACY-LOGS-0001` for the earlier historical logs reader and `DOC-WORKFLOW-LOGS-0001` for the later current logs-facing rule body. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `DOC-WORKFLOW-0001-CH-01` | `DOC-WORKFLOW-0001` | `introduced` | `none` | `DOC-WORKFLOW-0001-ST-01; DOC-WORKFLOW-0001-ST-02; DOC-WORKFLOW-0001-ST-03; DOC-WORKFLOW-0001-ST-04; DOC-WORKFLOW-0001-ST-05; DOC-WORKFLOW-0001-ST-06` | `2026-02-06T06:03:03Z` | `2026-04-10` | The first broad workflow parent release is being retained as a chronology-first contract with explicit clause identity so later narrower child packets do not erase the original pipeline boundary. | `S0A-2A-R01` | One initial release row is sufficient because this repair aligns the existing release to the current table model without changing the owned meaning. |
...
- The semantic start of this release is now anchored to the source issue creation time `2026-02-06T06:03:03Z`, while the release record itself entered repo chronology later on `2026-04-10`.
...
- The current reader boundary now also makes the logs and ADR layers explicit as delegated-summary clauses beneath the broader parent path, while keeping the parent-owned workflow summaries intact.
...
- `DOC-WORKFLOW-LEGACY-LOGS-0001` and `DOC-WORKFLOW-LOGS-0001` are now read here as the historical and current narrower logs readers respectively, while this parent keeps only the broad workflow-layer logs summary.
...
- The logs clause no longer reads as an isolated parent-owned clause: the parent keeps the broad workflow reading only, and the `S0A-2A` ledger plus the narrower logs contracts now carry the delegated reader path.
