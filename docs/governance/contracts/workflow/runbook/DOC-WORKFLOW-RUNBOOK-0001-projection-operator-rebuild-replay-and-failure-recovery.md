...
contract_record:
  ...
  notes:
    ...
    - `effective_from` is anchored to the source issue creation time `2026-02-06T06:03:03Z`; the later accepted SUP packet sharpens the child-opening justification but does not replace the earlier issue-level semantic anchor for the runbook layer.
```
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `DOC-WORKFLOW-RUNBOOK-0001-GOV-01` | `contribution-event` | `DOC-WORKFLOW-RUNBOOK-0001` | `unknown` | `family-introduced` | `2026-02-06T06:03:03Z` | `GitHub issue S0A-2A (#24)` | The issue-only source introduced the runbook slice on the issue creation date, but it does not defend a named current steward or approver for the current contract state. |
...
- The semantic start of this release is now anchored to the source issue creation time `2026-02-06T06:03:03Z`, while the later accepted SUP round on `2026-04-12` sharpens the child-opening evidence path inside the same release.
