...
# ===== repo.py (v3 with article-aware helpers) =====
from sqlalchemy import select, or_, delete, update, func
from models import SessionLocal, Entry, Source, EntrySource, Article
...
# ---------- add / update / upsert / delete (backward compatible) ----------
...
def get_article_sentences(article_id: int):
    with SessionLocal() as s:
        stmt = select(Entry.id, Entry.src_text, Entry.tgt_text, Entry.position, Entry.created_at)               .where(Entry.article_id == article_id)               .order_by(Entry.position.asc())
...
def insert_sentence(article_id: int, after_position: int, src: str, tgt: str,
                    ...
                    created_at: Optional[str] = None) -> int:
    """
    Insert a new sentence into an article after the given position.
    Shifts following positions by +1. Returns new entry_id.
    """
    src = (src or "").strip(); tgt = (tgt or "").strip()
    if not src or not tgt:
        raise ValueError("src/tgt required")
    with SessionLocal() as s:
        # Shift to open a slot
        s.execute(
            update(Entry)
        ...
        s.add(e); s.flush()
        # Optional link back to a Source
        src_obj = _ensure_source(s, source_name)
        if src_obj:
...
# ---------- batch insert (legacy, sentence-by-sentence) ----------
def add_batch_entries(zh_text: str, en_text: str,
                      source_name: Optional[str],
                      created_at: Optional[str] = None,
                      zh_delim: str = "。", en_delim: str = ".") -> List[int]:
    zh_units = [p.strip() for p in zh_text.split(zh_delim) if p.strip()]
    en_units = [p.strip() for p in en_text.split(en_delim) if p.strip()]
    n = min(len(zh_units), len(en_units))
    if n == 0:
        raise ValueError("No valid sentence pairs found")
    ids = []
    for zh, en in zip(zh_units[:n], en_units[:n]):
        ids.append(add_entry(src=zh, tgt=en, ls="zh", lt="en", source_name=source_name, created_at=created_at))
    return ids

# ---------- pattern builder + find_matches + bulk_replace ----------
