"""S2D-1B/P2-C1-S2: second projection onboarding runner.

This script orchestrates the S2D-1B labs for the legacy projection
`chronicle_events_to_entries` by running them as a single onboarding
"套餐" and appending a summary record under `artifacts/s2d-runs.json`.

In C1 the underlying labs were skeletons (known red). From C2
onwards the same runner is reused, but the labs are upgraded to carry
minimal real logic and can report `ok=true` for a happy path.
...
BACKFILL_SCRIPT = "backend/scripts/labs/s2d1b_chronicle_events_to_entries_backfill_smoke.py"
HARNESS_SCRIPT = "backend/scripts/labs/s2d1b_chronicle_events_to_entries_harness_drill.py"

BACKFILL_SCENARIO_ID = "s2d1b_chronicle_events_to_entries_backfill_smoke"
HARNESS_SCENARIO_ID = "s2d1b_chronicle_events_to_entries_harness_drill"
...
def _run_lab(script: str, *, database_url: str, run_id: str, outdir: Path) -> tuple[int, bool]:
    ...
    print(f"[S2D-1B] running lab: script={script} outdir={outdir}")
    ...
    print(f"[S2D-1B] lab finished: script={script} rc={rc} ok={ok}")
    return rc, ok
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="S2D-1B second projection onboarding runner")
    p.add_argument("--database-url", required=True, help="SQLAlchemy-style DATABASE_URL for dev/test Postgres")
    p.add_argument("--run-id", required=False, help="Optional explicit run id (default: YYYYMMDD-HHMMSS)")
    p.add_argument(
        "--snapshot-root",
        required=False,
        default="docs/labs/_snapshot/auto",
        help="Root directory for lab snapshot outputs (default: docs/labs/_snapshot/auto)",
    )
    p.add_argument("--phase", required=False, default="P2", help="Phase id for artifacts/s2d-runs.json (default: P2)")
    p.add_argument("--cycle", required=False, default="C1", help="Cycle id for artifacts/s2d-runs.json (default: C1)")
    p.add_argument("--step", required=False, default="S2", help="Step id for artifacts/s2d-runs.json (default: S2)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("[S2D-1B] onboarding skeleton run summary:")
