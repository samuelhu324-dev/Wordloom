"""S5B-3A hard gate entrypoint (membership audit coverage).

This script is wired by S0D-2A to provide a single command that:
- runs the S5B-3A membership_audit_coverage drills runner
- discovers the resulting run_dir from stdout or filesystem
- invokes the shared S5B-1A verifier against that run_dir
- appends a summary record into artifacts/s5b3a-runs.json
...
LOG_ID = "S5B-3A"
DEFAULT_SUITE_ID = "membership_audit_coverage"


def _run_runner(*, suite_id: str, env: dict[str, str]) -> Tuple[int, str, str]:
    cmd = [
        sys.executable,
        "scripts/drills/s5b3a_p2c1s1_drills_runner.py",
    ]
    p = subprocess.run(cmd, env=env, text=True, capture_output=True)

    combined = (p.stdout or "") + ("\n" if p.stdout and p.stderr else "") + (p.stderr or "")

    suite_root = Path("docs/labs/_snapshot/auto/S5B-3A") / suite_id
...
def main() -> int:
    ...
    suite_id = base_env.get("S5B_3A_SUITE_ID", DEFAULT_SUITE_ID)

    print("[S5B-3A] hard gate")
    ...
    append_run_record(Path("artifacts") / "s5b3a-runs.json", record)
