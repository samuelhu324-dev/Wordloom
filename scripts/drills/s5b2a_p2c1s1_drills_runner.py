"""S5B-2A P2-C1-S1 drills runner.

Goal (v1): add one repeatable drill suite for the selected critical chain `bookshelf.delete`
that proves:
- member (not_admin) cannot delete -> 403 + audit denied + reason=not_admin
- cross-tenant delete is blocked -> 403 + audit denied + reason=tenant_mismatch

Artifacts contract (S5B-2A P0-C1-S3):
- Writes S5B-1A-compatible recipe/result/metrics schemas so we can reuse the existing verifier.
- Output: docs/labs/_snapshot/auto/S5B-2A/<suite_id>/<run_id>/
...
Run:
  python scripts/drills/s5b2a_p2c1s1_drills_runner.py
...
def _artifact_run_dir(*, suite_id: str, run_id: str) -> str:
    return os.path.join("docs", "labs", "_snapshot", "auto", "S5B-2A", suite_id, run_id)
...
async def run() -> tuple[str, dict[str, Any]]:
    cfg = RunnerConfig(
        ...
        suite_id=os.getenv("S5B_2A_SUITE_ID", "bookshelf_delete_entrypoint"),
    )
    ...
    recipe = {
        ...
        "cases": [
            {
                "case_id": "delete_not_admin_403",
                "title": "member cannot delete (403 + audit denied + reason=not_admin)",
                "endpoint": {"method": "DELETE", "path_template": "/api/v1/bookshelves/{bookshelf_id}"},
            },
            {
                "case_id": "delete_tenant_mismatch_403",
                "title": "cross-tenant delete is rejected (403 + audit denied + reason=tenant_mismatch)",
                "endpoint": {"method": "DELETE", "path_template": "/api/v1/bookshelves/{bookshelf_id}"},
            },
        ],
    }
    ...
    try:
        async with httpx.AsyncClient(base_url=cfg.api_base_url, timeout=30.0) as client:
            # Setup: create libraries (owner = implicit dev user on API side)
            lib_a_name = f"s5b2a-lib-a-{run_id[:8]}"
            lib_b_name = f"s5b2a-lib-b-{run_id[:8]}"
            ...
            # Setup: grant roles
            r_grant_admin_a = await client.post(
                f"/api/v1/libraries/{lib_a_id}/memberships",
                json={"user_id": str(user_admin), "role": "admin"},
                headers={"X-Library-Id": str(lib_a_id)},
            )
            _require_status(r_grant_admin_a, ok_statuses={200, 201, 204}, label="grant_admin_a", log_lines=log_lines)

            r_grant_admin_b = await client.post(
                f"/api/v1/libraries/{lib_b_id}/memberships",
                json={"user_id": str(user_admin), "role": "admin"},
                headers={"X-Library-Id": str(lib_b_id)},
            )
            _require_status(r_grant_admin_b, ok_statuses={200, 201, 204}, label="grant_admin_b", log_lines=log_lines)
            ...
            _require_status(r_grant_member_a, ok_statuses={200, 201, 204}, label="grant_member_a", log_lines=log_lines)

            # Setup: create one bookshelf in library A (as owner implicit)
            shelf_name = f"s5b2a-shelf-a-{run_id[:8]}"
            ...
            # Case 1: member delete -> 403 not_admin
            delete_path = f"/api/v1/bookshelves/{shelf_id}"
            r_member_delete = await client.delete(
                delete_path,
                headers={
            ...
            member_req_id = r_member_delete.headers.get("X-Request-Id")
            member_audit_rows = await _fetch_audit_rows(engine=engine, request_id=member_req_id) if member_req_id else []
            case_results.append(
                _build_case_result(
                    case_id="delete_not_admin_403",
                    ...
                    inputs={
                        ...
                        "tenant_id": str(lib_a_id) if lib_a_id else None,
                        "actor_user_id": str(user_member),
                        "http": {"method": "DELETE", "path": delete_path, "path_template": "/api/v1/bookshelves/{bookshelf_id}"},
                    },
                    expected={
                        "http_status": 403,
                        "audit_expected": True,
                        "audit": {"action": "bookshelf.delete", "result": "denied", "reason": "not_admin"},
                        "audit_required_fields": ["tenant_id", "actor_user_id", "request_id", "action", "result", "reason"],
                    },
                    observed={
                        "http_status": int(r_member_delete.status_code),
                        "audit_rows": {"count": len(member_audit_rows), "rows": member_audit_rows[:10]},
                    },
                )
            )

            # Case 2: cross-tenant delete -> 403 tenant_mismatch
            r_cross_delete = await client.delete(
                delete_path,
                headers={
            ...
            cross_req_id = r_cross_delete.headers.get("X-Request-Id")
            cross_audit_rows = await _fetch_audit_rows(engine=engine, request_id=cross_req_id) if cross_req_id else []
            case_results.append(
                _build_case_result(
                    case_id="delete_tenant_mismatch_403",
                    title="cross-tenant delete is rejected",
                    inputs={
                    ...
                    expected={
                        "http_status": 403,
                        "audit_expected": True,
                        "audit": {"action": "bookshelf.delete", "result": "denied", "reason": "tenant_mismatch"},
                        "audit_required_fields": ["tenant_id", "actor_user_id", "request_id", "action", "result", "reason"],
                    },
                    observed={
                        "http_status": int(r_cross_delete.status_code),
                        "audit_rows": {"count": len(cross_audit_rows), "rows": cross_audit_rows[:10]},
                    },
                )
            )
        ...
        for case_id, title in (
            ("delete_not_admin_403", "member cannot delete"),
            ("delete_tenant_mismatch_403", "cross-tenant delete is rejected"),
        ):
            if not any(r.get("case_id") == case_id for r in case_results):
                case_results.append(
                    {
                        "case_id": case_id,
                        "title": title,
